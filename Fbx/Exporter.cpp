...
Fbx::Exporter::Exporter(wstring file, bool bXna)
	: file(file), bXna(bXna)
{
...
void Fbx::Exporter::ExportMeshData(wstring saveFolder, wstring saveName)
{
...
void Fbx::Exporter::ReadMeshData(FbxNode * node, int parentBone)
{
	...
	for (int p = 0; p < mesh->GetPolygonCount(); p++)
	{
		...
		for (int vi = vertexInPolygonCount - 1; vi >= 0; vi--)
		{
...
void Fbx::Exporter::ReadAnimation()
{
	...
	for (int i = 0; i < importer->GetAnimStackCount(); i++)
	{
		...
		animation->TotalFrame = (end - start) + 1;
